...
if __name__ == '__main__':
    ...
    prior_cov = np.array([[1e-5, 0.0, 0.0], [0.0, 1e-5, 0.0], [0.0, 0.0, 1e-5]])
    ...
    for case in cases:
        ...
        graph, gt = read_data_2d(graph, origin_path + 'slam.g2o', None, 5, prior_cov, threshold,
                                 falseLoop=False)
        #graph, gt = read_kitti(graph, origin_path + 'slam.g2o', None, 5, prior_cov, threshold, falseLoop=False)
        ...
        for i in range(300):
            ...
            if i % 1 == 0:
                ...
                for pair in pairs:
                    plt.plot([poses_iter_1[pair[0]][0], poses_iter_1[pair[1]][0]],
                             [poses_iter_1[pair[0]][1], poses_iter_1[pair[1]][1]], '--', color='gray')
            ...
            if i % 1 == 0:
                ...
                if gt is not None:
                    ...
                    print("Iteration ", i, " ", "Propagation ", np.round(__time - _time, 3), " Update ",
                          np.round(____time - ___time, 3), " NB all gaussians ", nb_ga)
